...
<form action ="save_user.asp?action=xg&id=<%=id%>&qx=<%=id%>" method="post" name="addFrm" onSubmit="return checkform()">
  <table border="0" cellspacing="1" cellpadding="4" align="center" bgcolor="#6298E1" width="100%">
    ...
    <tr>
      <td height=30 class="classtd"><div align="right"><strong>用户名：</strong></div></td>
      <td class="classtd"><%=rs("admin_id")%></td>
    </tr>
    <tr>
...
<% if Instr(session("juese"),"|306,")=0 then %> 
    <tr>
      ...
      <td class="classtd"><%=rs("rank")%></td>
    </tr>
    <tr>
      <td height=30 class="classtd"><div align="right" ><strong>角&nbsp;色：</strong></div></td>
      <td class="classtd">
		<%
...
<% end if %>
    <tr>
      <td height=30 class="classtd"><div align="right"><strong><font color="red">*</font><%if session("rank")="IT" then%>密&nbsp;码<% else %>旧密码<% end if %>：</strong></div></td>
      <td class="classtd"><input name="userpw" type="password" class="form"  value="<%=rs("admin_pw")%>" size="20" style="width: 180px;"><%if session("rank")="IT" then%>(不改密码请保持原有数据)<%end if%></td>
    </tr>
<%if Instr(session("juese"),"|306,")=0 then%>
    <tr>
