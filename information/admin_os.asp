...
<table border="0" cellspacing="1" cellpadding="4" bgcolor="#6298E1" align="center" width="100%">
  ...
  <tr class=classtop1>
    ...
    <td nowrap background="../images/th_bg.gif" height="15" align="center"><strong>型号</strong></td>
  ...
  <form name="form2" method="post" action="admin_os.asp?action=add">
    <tr>
      <td height="15" class="classtd"><div align="center">
          <input name="s_name" type="text" disabled="disabled" class="form" id="s_name" value="新建" size="5">
        </div></td>
      <td height="15" class="classtd"><div align="center">
  ...
  <%if rs.eof and rs.bof then
        response.write "<center><font color=red>没有任何记录！有需要请在下方添加！</font></center>"
        Else
  ...
  <tr>
    <form name="form1" method="post" action="admin_os.asp?action=edit&s_id=<%=rs("id")%>">
      ...
      <td class="classtd"><div align="center">
          ...
          <input name="Submit2" type="button" class="button" onclick="{if(confirm('确认删除么?')){location.href='admin_os.asp?action=del&s_id=<%=rs("id")%>';}return false;}" value="删 除">
        </div></td>
    </form>
  </tr>
  <%
...
%>
  <form action="admin_os.asp" method="post">
    <tr class=botbg>
      <td height="15" align="center" colspan="4"><div align="center"> 共有 <b><%=rs.recordcount%></b> 条记录, 页次: <b><font color=red><%=pagecount%></font>/<%=rs.pagecount%></b>, 
        当前从第
        <%
           if pagecount<=1 then
...
%>
  <tr class=classtop1>
    <td height="15" colspan="4" class="classtop1">排序编号将影响排序，数值越小越靠前。</td>
  </tr>
</table>
<%
end if%>
