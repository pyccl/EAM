...
<%
   ...
   %>
			...
			<div align="center">
				页次：<font color="red"><%=epage%></font>/<font color="red"><%=rsuu.pagecount%></font>页，共有<font color="red"><%=rsuu.recordcount%></font>条记录。
				<a href="asset.asp?id=<%=rsu("id")%>&page=1">首页</a>&nbsp;
				<a href="asset.asp?id=<%=rsu("id")%>&page=<%=epage-1%>">前一页</a>&nbsp;
				<a href="asset.asp?id=<%=rsu("id")%>&page=<%=epage+1%>">后一页</a>&nbsp;
				<a href="asset.asp?id=<%=rsu("id")%>&page=<%=rsuu.pagecount%>">末页</a>
			</div>
		<%end if
		   end if%>
	</td>
